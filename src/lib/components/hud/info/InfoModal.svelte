...
<Modal
  bind:open={show}
  classDialog="absolute pointer-events-auto"
  class="{$infoApi.smStep && `modal ${$infoApi.smStep.label}`} text-black p-2"
  autoclose
  outsideclose
>
  <div class="absolute -top-2 left-4 flex items-center gap-1">
    {#if $infoApi.smStep}
      ...
      {/if}
      <Lol key={$infoApi.smStep.titleKey} class="font-bold" />
    {:else}
